...
int UsbB204 :: UsbPortSet()
{
    ...
    //FT_SetTimeouts((FT_HANDLE)ftHandle,1000,1000);              // タイムアウト設定 1sec
    int res=UsbPortSetSub(&configData);
    FT_SetTimeouts((FT_HANDLE)ftHandle,2000,2000);              // タイムアウト設定 2sec    220429
	return(res);
...
int UsbB204 :: UsbResponseRead( int* FPGAver, int* pll_lock, unsigned char* data)
{
	...
	//[data b c]=mxRead(ftHandle,4);  %引数　ポート番号、読み出すバイト数  戻り値  1:データ , 2:バイト数, 3:FT_Readのステータス
	FT_Read( (FT_HANDLE)ftHandle,data,4,&bytesRead);
	bytes=data[2]*256+data[3]+1;
	if( bytes%2 )
		bytes=bytes+1;

	//[data2 b c]=mxRead(ftHandle,bytes);  %引数　ポート番号、読み出すバイト数  戻り値  1:データ , 2:バイト数, 3:FT_Readのステータス
	...
	if(data[0]==02 && data[1]==255) {  // % 0x02 ff
		if(data[4]==1){  // % 通信結果応答  0x02 ff 00 03 01 
			if(data[5]==0){
				OK=1;
			}
			else{
		...
		else if(data[4]==2){  //% Status要求応答    0x02 ff 00 03 02
			*FPGAver=(int)(data[5]);
			*pll_lock=(int)(data[6]);
			OK=1;
